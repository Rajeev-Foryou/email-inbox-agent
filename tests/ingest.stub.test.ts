...
class FakeEmailClient {
  ...
  async markAsSeen(_uids: number | number[]): Promise<void> { return; }
}
...
class SpyRepository {
  ...
  async isUidProcessed(_mailbox: string, _uid: number): Promise<boolean> {
    return false;
  }
  async findByImapUid(_mailbox: string, _uid: number): Promise<any> {
    return null;
  }
}
